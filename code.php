...
if (isset($_POST['user_claims_btn'])) {
    ...
    elseif ($roles=='super_admin') {
        $auth->setCustomUserClaims($uid, ['super_  admin' => true]);
        $msg = "user role as super admin";
    }

    elseif ($roles=='norole') {
        $auth->setCustomUserClaims($uid, null);
        $msg = "user role removedn";
    }
...
if (isset($_POST['change_password_btn'])) {
    ...
    if ($new_password == $retype_password) {
        ...
        if($updatedUser) {
            $_SESSION['status'] = "Pw Updated";
            header('Location: user-list.php');
            exit();
        }
        else {
            $_SESSION['status'] = "Pw Not Updated";
            header('Location: user-list.php');
            exit();
        } 
    }
...
if(isset($_POST['save_contact'])) {
    ...
    if($postRef_result) {
        ...
        $_SESSION['status'] = "Contact Not Added Successfully";
        header ('Location: index.php');
    }
}
